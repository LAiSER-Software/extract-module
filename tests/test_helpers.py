...
def eda_on_results(
    ...
    # --- top / bottom matches ---
    top_matches = (
        df.sort_values(by="Correlation Coefficient", ascending=False).head(10)
        if "Correlation Coefficient" in df.columns
        else pd.DataFrame()
    )
    bottom_matches = (
        df.sort_values(by="Correlation Coefficient", ascending=True).head(10)
        if "Correlation Coefficient" in df.columns
        else pd.DataFrame()
    )
    ...
    if "taxonomy" in df.columns and "Correlation Coefficient" in df.columns:
        agg_df = (
            df.groupby("taxonomy")["Correlation Coefficient"]
    ...
    if print_report:
        ...
        if not bottom_matches.empty:
            ...
            cols_show = [
                ...
                if c in bottom_matches.columns
            ]
