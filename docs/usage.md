...
| `"syllabus"`, `"course_syllabus"`, `"course_syllabi"` | courses and programs | the `description` and `learning_outcomes` columns |

!!! note "Syllabus columns are fixed"
    For syllabus input the prompt treats the course description and its learning outcomes separately, so
    it reads the `description` and `learning_outcomes` columns by name regardless of `text_columns`.
...
| `top_k` | `25` | maximum aligned matches returned per document |
...
By default `extract_concepts` returns a `DataFrame` with one row per match:
...
```python
extractor.extract_concepts(..., output_csv_path="results.csv")
```
...
| Parameter | Default | Description |
|---|---|---|
| `batch_size` | `32` | documents processed per batch |
