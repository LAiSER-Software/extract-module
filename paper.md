...
Job postings, course syllabi, and credential descriptions all describe skills, but each describes them in its own words. One employer asks for "proficiency in Python," another for "experience building data pipelines," and a college course promises "programming fundamentals." A person reading these can see which ones overlap; software cannot, and neither can the large-scale analyses that educators, employers, and policymakers rely on to judge whether what is taught matches what is hired for.

LAiSER is an open-source Python tool that reads text of this kind, uses large language models (LLMs) to identify the skills it describes, and matches each one to an entry in a published catalogue of skills. Such catalogues, known as skill taxonomies, give every skill a standard name and a stable identifier, so that the three phrasings above resolve to the same entry and become countable and comparable across thousands of documents. LAiSER aligns to the European Skills, Competences, Qualifications and Occupations classification (ESCO) [@ESCO2020] among other taxonomies, and can additionally sort what it finds into knowledge, skills, and abilities (KSA) and estimate the level at which each is required, using the Scottish Credit and Qualifications Framework (SCQF) [@SCQF2019].

The intended users are researchers and practitioners who need to analyse skill language at a scale that makes reading by hand impractical: labor economists studying how occupations change, curriculum designers checking a program against industry demand, and credentialing bodies issuing standards-aligned badges. LAiSER runs on a laptop, on a shared GPU cluster, or against a hosted model service, so the same analysis is available whether or not a research group has dedicated hardware. The source code is available at [https://github.com/LAiSER-Software/extract-module](https://github.com/LAiSER-Software/extract-module) under the MIT License.
